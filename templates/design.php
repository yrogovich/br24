<?php
/*
*   Template name: Дизайн
*/
get_header();
?>
<div class="design-page">
<header>
    <div class="container">
        <div class="hero">
            <div class="hero-left">
                <h1 class="h1">
                    <span class="d-block">Фирменный стиль и</span> рекламный дизайн
                </h1>

                <ol class="style-1">
                    <li>
                        <div class="block">
                            <span class="title">Маркетинг-кит</span>
                        </div>
                    </li>
                    <li>
                        <div class="block">
                            <span class="title">Логотип<span class="d-block"></span>
                        </div>
                    </li>
                    <li>
                        <div class="block">
                            <span class="title">Наружная реклама</span>
                        </div>
                    </li>
                    <li>
                        <div class="block">
                            <span class="title">Полиграфия и упаковка</span>
                        </div>
                    </li>
                </ol>

                <a href="#popup-request" rel="modal:open" class="btn btn-primary btn-lg btn-shadow">Узнать стоимость</a>
            </div>
            <div class="hero-right">
                <img src="<?php bloginfo('template_url') ?>/src/img/design/main-img.png" alt="Создаем сайты для бизнеса">
            </div>
        </div>
    </div>
</header>

<div class="section" id="seo-cases">
    <div class="container">
        <div class="h2">стоимость</div>
        <div class="h2-primary">Наши кейсы и цены</div>
        
        <div class="design-cases">
            <div class="design-case">
                <div class="head">
                    <div class="primary h3">Маркетинг Кит</div>

                    <div class="pair">
                        <div class="column">
                            <div class="price">2 000 Руб.</div>
                            <div class="small-text">за 1 страницу</div>
                        </div>

                        <a href="#popup-kit" rel="modal:open" class="btn btn-primary btn-opacity">Заказать</a>
                    </div>
                    <!-- /.pair -->
                </div>
                <!-- /.head -->

                <div class="body">
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/kit/FXLuminare.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/kit/thumbs/FXLuminare.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/kit/sell-flat.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/kit/thumbs/sell-flat.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/kit/225000.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/kit/thumbs/225000.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/kit/money.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/kit/thumbs/money.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                </div>
                <!-- /.body -->
            </div>
            <!-- /.design-case -->

            <div class="design-case">
                <div class="head">
                    <div class="primary h3">Логотип</div>

                    <div class="pair">
                        <div class="column">
                            <div class="price">10 000 Руб.</div>
                            <div class="small-text">за 3 варианта</div>
                        </div>

                        <a href="#popup-logo" rel="modal:open" class="btn btn-primary btn-opacity">Заказать</a>
                    </div>
                    <!-- /.pair -->
                </div>
                <!-- /.head -->

                <div class="body">
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/logo/alkana.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/logo/alkana.png" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/logo/balash.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/logo/balash.png" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/logo/markizy.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/logo/markizy.png" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/logo/klimolog.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/logo/klimolog.png" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/logo/kitauto.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/logo/kitauto.png" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/logo/portfel.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/logo/portfel.png" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/logo/oboitown.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/logo/oboitown.png" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/logo/r-com.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/logo/r-com.png" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                </div>
                <!-- /.body -->
            </div>
            <!-- /.design-case -->

            <div class="design-case">
                <div class="head">
                    <div class="primary h3">Наружная реклама</div>

                    <div class="pair">
                        <div class="column">
                            <div class="price">от 5 000 Руб.</div>
                            <div class="small-text">за 3 варианта</div>
                        </div>

                        <a href="#popup-outad" rel="modal:open" class="btn btn-primary btn-opacity">Заказать</a>
                    </div>
                    <!-- /.pair -->
                </div>
                <!-- /.head -->

                <div class="body">
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/outad/econom.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/outad/econom.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->    
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/outad/pich.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/outad/pich.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/outad/bacauto.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/outad/bacauto.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/outad/rancho.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/outad/rancho.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                </div>
                <!-- /.body -->
            </div>
            <!-- /.design-case -->

            <div class="design-case">
                <div class="head">
                    <div class="primary h3">Упаковка продукции</div>

                    <div class="pair">
                        <div class="column">
                            <div class="price">от 4 000 Руб.</div>
                            <div class="small-text">за 1 макет</div>
                        </div>

                        <a href="#popup-package-design" rel="modal:open" class="btn btn-primary btn-opacity">Заказать</a>
                    </div>
                    <!-- /.pair -->
                </div>
                <!-- /.head -->

                <div class="body">
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/package/pack-1.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/package/pack-1.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->    
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/package/pack-2.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/package/pack-2.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/package/pack-3.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/package/pack-3.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/package/pack-4.jpg" class="card wide">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/package/pack-4.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                </div>
                <!-- /.body -->
            </div>
            <!-- /.design-case -->
        </div>
        <!-- /.design-cases -->

        <div class="design-case">
                <div class="head">
                    <div class="primary h3">Веб дизайн</div>

                    <div class="pair">
                        <div class="column">
                            <div class="price">от 20 000 Руб.</div>
                        </div>

                        <a href="#web-package-design" rel="modal:open" class="btn btn-primary btn-opacity">Заказать</a>
                    </div>
                    <!-- /.pair -->
                </div>
                <!-- /.head -->

                <div class="body">
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/web/web-1.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/web/thumbs/web-1.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/web/web-2.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/web/thumbs/web-2.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/web/web-3.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/web/thumbs/web-3.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/web/web-4.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/web/thumbs/web-4.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/web/web-5.jpg" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/web/thumbs/web-5.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/web/web-6.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/web/thumbs/web-6.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/web/web-7.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/web/thumbs/web-7.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                    <a href="<?php bloginfo('template_url') ?>/src/img/design/web/web-8.png" class="card">
                        <img data-src="<?php bloginfo('template_url') ?>/src/img/design/web/thumbs/web-8.jpg" class="lazy" alt="photo">
                    </a>
                    <!-- /.card -->
                </div>
                <!-- /.body -->
            </div>
            <!-- /.design-case -->
        </div>
        <!-- /.design-cases -->
        
    </div>
</div>
<!-- /#services.section -->

<div class="section" id="services">
    <div class="container">
        <div class="h2">Разделы</div>
        <div class="h2-primary">Уверены, вам это тоже будет интересно</div>
        <div class="service-container">
            <?php get_template_part('templates/service-cards/site-creating') ?>
            <?php get_template_part('templates/service-cards/context-ad') ?>
            <?php get_template_part('templates/service-cards/tech-maintence') ?>
            <?php get_template_part('templates/service-cards/video-creating') ?>
        </div>
        <!-- /.service-container -->       
    </div>
</div>
<!-- /#services.section -->
</div>

<!-- PopUp -->
<div id="popup-kit" class="overlay">
    <div class="popup">
        <?php get_template_part('templates/parts/modal-head') ?>
        
        <div class="content">
            <div class="title">
                <div class="h">Оставить заявку на маркейтинг кит</div>
                <div class="sub-h">Оставьте свои контакты, что бы мы могли связаться с Вами</div>
            </div>
            <form action="javascript:" onsubmit="callHandler('#kit-form')" id="kit-form">
                <input type="hidden" name="url" value="<?php echo $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] ?>">
                <input type="hidden" name="form" value="Оставить заявку на маркейтинг кит">
                <div class="input-wrapper icon i-user">
                    <input type="text" placeholder="Введите ваше имя" name="user_name" required>
                </div>
                <div class="input-wrapper icon i-phone">
                    <input type="text" placeholder="Ваш № телефона" name="user_phone" class="phone-mask" required>
                </div>
                <div class="input-wrapper icon i-mail">
                    <input type="mail" placeholder="Введите ваш e-mail">
                </div>
                <button class="btn btn-form btn-shadow">Оставить заявку</button>
            </form>
        </div>
    </div>
</div>
<!-- /PopUp -->
<!-- PopUp -->
<div id="popup-logo" class="overlay">
    <div class="popup">
        <?php get_template_part('templates/parts/modal-head') ?>
        
        <div class="content">
            <div class="title">
                <div class="h">Оставить заявку на логотип</div>
                <div class="sub-h">Оставьте свои контакты, что бы мы могли связаться с Вами</div>
            </div>
            <form action="javascript:" onsubmit="callHandler('#logo-form')" id="logo-form">
                <input type="hidden" name="url" value="<?php echo $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] ?>">
                <input type="hidden" name="form" value="Оставить заявку логотип">
                <div class="input-wrapper icon i-user">
                    <input type="text" placeholder="Введите ваше имя" name="user_name" required>
                </div>
                <div class="input-wrapper icon i-phone">
                    <input type="text" placeholder="Ваш № телефона" name="user_phone" class="phone-mask" required>
                </div>
                <div class="input-wrapper icon i-mail">
                    <input type="mail" placeholder="Введите ваш e-mail">
                </div>
                <button class="btn btn-form btn-shadow">Оставить заявку</button>
            </form>
        </div>
    </div>
</div>
<!-- /PopUp -->
<!-- PopUp -->
<div id="popup-outad" class="overlay">
    <div class="popup">
        <?php get_template_part('templates/parts/modal-head') ?>
        
        <div class="content">
            <div class="title">
                <div class="h">Оставить заявку на наружную рекламу</div>
                <div class="sub-h">Оставьте свои контакты, что бы мы могли связаться с Вами</div>
            </div>
            <form action="javascript:" onsubmit="callHandler('#outad-form')" id="outad-form">
                <input type="hidden" name="url" value="<?php echo $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] ?>">
                <input type="hidden" name="form" value="Оставить заявку на наружную рекламу">
                <div class="input-wrapper icon i-user">
                    <input type="text" placeholder="Введите ваше имя" name="user_name" required>
                </div>
                <div class="input-wrapper icon i-phone">
                    <input type="text" placeholder="Ваш № телефона" name="user_phone" class="phone-mask" required>
                </div>
                <div class="input-wrapper icon i-mail">
                    <input type="mail" placeholder="Введите ваш e-mail">
                </div>
                <button class="btn btn-form btn-shadow">Оставить заявку</button>
            </form>
        </div>
    </div>
</div>
<!-- /PopUp -->
<!-- PopUp -->
<div id="popup-package-design" class="overlay">
    <div class="popup">
        <?php get_template_part('templates/parts/modal-head') ?>
        
        <div class="content">
            <div class="title">
                <div class="h">Оставить заявку на упаковку продукции</div>
                <div class="sub-h">Оставьте свои контакты, что бы мы могли связаться с Вами</div>
            </div>
            <form action="javascript:" onsubmit="callHandler('#package-design-form')" id="package-design-form">
                <input type="hidden" name="url" value="<?php echo $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] ?>">
                <input type="hidden" name="form" value="Оставить заявку на упаковку продукции">
                <div class="input-wrapper icon i-user">
                    <input type="text" placeholder="Введите ваше имя" name="user_name" required>
                </div>
                <div class="input-wrapper icon i-phone">
                    <input type="text" placeholder="Ваш № телефона" name="user_phone" class="phone-mask" required>
                </div>
                <div class="input-wrapper icon i-mail">
                    <input type="mail" placeholder="Введите ваш e-mail">
                </div>
                <button class="btn btn-form btn-shadow">Оставить заявку</button>
            </form>
        </div>
    </div>
</div>
<!-- /PopUp -->
<!-- PopUp -->
<div id="web-package-design" class="overlay">
    <div class="popup">
        <?php get_template_part('templates/parts/modal-head') ?>
        
        <div class="content">
            <div class="title">
                <div class="h">Оставить заявку на веб дизайн</div>
                <div class="sub-h">Оставьте свои контакты, что бы мы могли связаться с Вами</div>
            </div>
            <form action="javascript:" onsubmit="callHandler('#web-design-form')" id="web-design-form">
                <input type="hidden" name="url" value="<?php echo $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] ?>">
                <input type="hidden" name="form" value="Оставить заявку на веб дизайн">
                <div class="input-wrapper icon i-user">
                    <input type="text" placeholder="Введите ваше имя" name="user_name" required>
                </div>
                <div class="input-wrapper icon i-phone">
                    <input type="text" placeholder="Ваш № телефона" name="user_phone" class="phone-mask" required>
                </div>
                <div class="input-wrapper icon i-mail">
                    <input type="mail" placeholder="Введите ваш e-mail">
                </div>
                <button class="btn btn-form btn-shadow">Оставить заявку</button>
            </form>
        </div>
    </div>
</div>
<!-- /PopUp -->


<script>
    (function($){   
         /**
         * When DOM is ready
         */
        $(document).ready(function(){
            //lightbox
            if($('.body .card')) {
                $('.body .card').simpleLightbox();
            }
        });
    })(jQuery); // <----- jQuery no conflict wrapper
</script>
<?php get_footer(); ?>